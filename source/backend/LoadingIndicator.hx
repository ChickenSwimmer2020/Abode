...
class LoadingIndicator extends ASprite {
    public var loadingSpeed:Float=0.2; //tween time
    ...
    public function new(x:Float,y:Float) {
        ...
        for (seg in segments) addChild(seg).alpha=0.3; //two birds, one line.
        animateNext();
    }
    ...
    private function drawOutlines() {
        var w:Float = 65;
        var h:Float = 65;
        m.translate(-w / 2, -h / 2); // move origin to center
        m.rotate(angle);              // rotate around that center
        m.translate(w / 2, h / 2);   // move back
...
// Each segment knows which "corner" it is
...
class LoadingSegment extends ASprite {
    ...
    private function drawSegment(pos:SegmentPosition) { //i meant to make it look like the original but i accidently made it much cooler
        var cx = W / 2;
        var cy = H / 2;
